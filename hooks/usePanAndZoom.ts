...
export const usePanAndZoom = () => {
  ...
  const handleTouchMove = useCallback((e: React.TouchEvent, containerRect: DOMRect) => {
      ...
      if (e.touches.length === 2 && lastTouchDist.current > 0 && lastPanPoint.current) {
          ...
          // Capture current values to avoid null property access if the ref is cleared before state update runs
          const capturedLastPanPoint = lastPanPoint.current ? { ...lastPanPoint.current } : null;
          if (!capturedLastPanPoint) return;

          const panDelta = {
              x: newCenter.x - capturedLastPanPoint.x,
              y: newCenter.y - capturedLastPanPoint.y
          };
          ...
          setPanOffset(prevPan => {
              ...
              const anchor = {
                  x: capturedLastPanPoint.x - containerRect.left,
                  y: capturedLastPanPoint.y - containerRect.top
              };
