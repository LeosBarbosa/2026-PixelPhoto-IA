...
export type ToolId =
  ...
  // Workflow
  | 'batchProcessing' | 'magicWorkflow' | 'confidentStudio' | 'photoStudio' | 'aiPortraitStudio' | 'funkoPopStudio' | 'tryOn' | 'styledPortrait' | 'superheroFusion' | 'doubleExposure' | 'polaroid'
  // Editing
  | 'crop' | 'adjust' | 'localAdjust' | 'magicMontage' | 'objectRemover' | 'removeBg' | 'faceSwap' | 'generativeEdit' | 'text' | 'textEffects' | 'aiTextEdit' | 'photoRestoration' | 'upscale' | 'superResolution' | 'unblur' | 'sharpen' | 'relight' | 'style' | 'trends' | 'portraits' | 'lowPoly' | 'pixelArt' | 'styleGen' | 'texture' | 'dustAndScratches' | 'extractArt' | 'neuralFilters' | 'history' | 'newAspectRatio' | 'denoise' | 'faceRecovery' | 'imageVariation' | 'outpainting' | 'productPhotography' | 'architecturalViz' | 'interiorDesign' | 'sketchRender' | 'vectorConverter' | 'bananimate' | 'aiPngCreator' | 'imageAnalysis';
...
export interface EditorContextType {
  ...
  handleFaceSwap: (sourceImageFile: File, target: DetectedObject | File, prompt: string, negativePrompt: string) => Promise<string | undefined>;
  ...
  commitChange: (file: File, toolId: ToolId, params?: any) => void;
  ...
  currentState: HistoryStateSnapshot | undefined;
}
